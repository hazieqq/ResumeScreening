...
class DB():
    ...
    def insertIntoJobPost(title,category,jobtype,vacancy,experience,closedDate,postedDate,salaryFrom,salaryTo,qualification,description,status1):
        try:
            cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
            cursor.execute('INSERT INTO jobpost VALUES (NULL, % s, % s, % s,% s,% s,% s,% s,% s,% s,%s,%s)', (title,jobtype,description,qualification,category,salaryFrom,status1,experience,postedDate,closedDate,salaryTo))
